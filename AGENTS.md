...
実ISOの対象hashと6項目の受入は`distribution/evidence/debian13/accepted-09/README.ja.md`。BIOS/UEFI/Secure Boot、実日本語入力、オフライン／オンライン導入と再起動、通常ミラーの署名付きAPT索引取得を確認した。未接続の独自機能まで完成扱いにしない。
...
## 固定した製品方針
2026-09-08の利用者指示により、実配布のベースはDebian 13 Trixie。保守性を優先し、上流ソースへ独自パッチを当てず、APT/dpkg・live-build・Debian Installerを利用する。Nia固有部分を独立DEBとして管理する。配布入力は`distribution/image/`と`distribution/packaging/`、理由は`distribution/docs/decisions/0001-debian13.ja.md`。従来の`distribution/profiles/nia-os.json`とcontracts/toolsはForky/Niaカタログ研究モデルとして保存し、実イメージの仕様や受入結果へ読み替えない。7コンポーネントのAPI・永続形式・検査を強引に変更してOSを起動させない。7リポジトリは独立維持する。
